...
const SIDEBAR_ITEMS: SidebarItem[] = [
  ...
  { name: "Explore Bounties", href: "/bounties", icon: Coins },
  { name: "Leaderboard", href: "/analytics", icon: Trophy },
  { name: "Jobs Marketplace", href: "/jobs", icon: Briefcase },
  { name: "Career Copilot", href: "/copilot", icon: Brain, badge: "AI" },
      ...
      `}>
        ...
        <div className="flex h-16 items-center justify-between px-6 border-b border-neutral-900">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-blue-600 to-indigo-600 font-bold text-white shadow-md shadow-blue-600/30">
              BF
            </div>
            <div>
              <span className="font-bold text-sm text-white">BountyForge</span>
              <span className="block text-[10px] text-neutral-500 font-medium">Contributor Space</span>
            </div>
          </div>
          <Button 
            variant="ghost" 
        ...
        {/* Workspace Quick switch or indicator */}
        <div className="px-4 py-3 border-b border-neutral-900">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800 text-left">
            <div className="h-5 w-5 rounded bg-blue-500/20 text-blue-400 font-bold flex items-center justify-center text-xs">
              D
            </div>
            <span className="font-medium text-xs text-neutral-300 truncate">Developer Portal</span>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Icon className={`h-4 w-4 shrink-0 transition-colors ${isActive ? "text-blue-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                  <span>{item.name}</span>
                </div>
                {item.badge && (
                  <Badge 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <div className="overflow-hidden">
                <span className="block text-xs font-semibold text-white truncate">{session?.user?.name || "Developer"}</span>
                <span className="block text-[10px] text-neutral-500 truncate">{session?.user?.email || "contributor@bountyforge.com"}</span>
              </div>
            </div>
            <button 
        ...
        <header className="h-16 border-b border-neutral-900 bg-neutral-950/80 backdrop-blur px-6 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs text-neutral-500 font-medium">
              ...
              <span className="text-neutral-300">My Dashboard</span>
            </div>
          </div>

          <div className="flex items-center gap-4">
            {/* Find Bounties Quick link */}
            <Link href="/bounties">
              <Button className="bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs h-9 px-4 flex items-center gap-2 shadow-lg shadow-blue-900/10">
                Explore Bounties
              </Button>
            </Link>
