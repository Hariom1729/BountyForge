...
import { Mail, UserCircle2, Code2, Building2 } from "lucide-react";
...
export default function SignInPage() {
  ...
  const handleProviderLogin = (provider: string) => {
    // We pass the role to our custom callback API route
    signIn(provider, { callbackUrl: `/api/auth/setup?role=${role}` });
  };

  const handleGuestLogin = () => {
    signIn("credentials", { role: "GUEST", callbackUrl: "/api/auth/setup" });
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-muted/20">
      <Card className="w-full max-w-md shadow-2xl border-border/50">
        <CardHeader className="text-center pb-6">
          <CardTitle className="text-3xl font-bold tracking-tight">Join BountyForge</CardTitle>
          <CardDescription className="text-base mt-2">
            Select your path and connect your account.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">1. Choose your role</p>
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-card px-2 text-muted-foreground font-semibold">2. Connect Account</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <Button 
              className="w-full h-12 text-base flex items-center justify-center gap-3 bg-foreground text-background hover:bg-foreground/90" 
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground font-semibold">Or Explore</span>
              </div>
            </div>
            ...
            <Button 
              type="button"
              variant="secondary" 
              className="w-full h-12 text-base flex items-center justify-center gap-3 bg-muted hover:bg-muted/80 text-foreground border border-transparent hover:border-border transition-colors"
